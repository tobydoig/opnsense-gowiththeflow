-- Two hourly buckets for the same (local, peer) pair -- should SUM together in History.
...
VALUES (strftime('%s','now') - 7200, 'tcp', '10.0.0.9', '172.66.147.243', 0, 'example.com', 'dns', 40000, 3000, 30, 20, 3);
INSERT INTO rollup_hourly (bucket_start, proto, local_ip, peer_ip, peer_is_local, peer_hostname, hostname_source, bytes_in, bytes_out, pkts_in, pkts_out, conn_count)
...
INSERT INTO rollup_hourly (bucket_start, proto, local_ip, peer_ip, peer_is_local, peer_hostname, hostname_source, bytes_in, bytes_out, pkts_in, pkts_out, conn_count)
VALUES (strftime('%s','now') - 1800, 'tcp', '10.0.0.42', '20.190.159.4', 0, 'login.microsoftonline.com', 'dns', 5000, 2000, 4, 3, 1);
...
-- (nvr) is the numerically larger member. Filtering History by
-- local_host=10.0.0.20 exercises the fix for finding 1a (this row would
-- previously have been silently dropped since it never appears as
-- local_ip for this pair).
INSERT INTO rollup_hourly (bucket_start, proto, local_ip, peer_ip, peer_is_local, category, bytes_in, bytes_out, pkts_in, pkts_out, conn_count)
VALUES (strftime('%s','now') - 3600, 'tcp', '10.0.0.9', '10.0.0.20', 1, 'Internal', 12000, 800, 15, 9, 2);
